...
The window title shows the fork's number with upstream's in parentheses — `v1.2.40 (4.1.0)` — from
`BuildInfo.pelerVersion`/`pelerDisplayName`, generated by `genBuildInfo` from `pelerAppVersion`. A
local build passes no override, so it falls back to upstream's number and the title reads as it
always did. **Do not reach for these where the upstream version is meant**: `BuildInfo.version` is
written into every saved file and `displayName` into file headers and generated VHDL.
